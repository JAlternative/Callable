...
def testing_step(stage_name, stand_address, tasks) {
    ws("${stage_name}") {
        ...
        warnError('Step failed') {
            dir('Selenium') {
                sh "./gradlew --configure-on-demand -DXmx1g -DXX:MaxMetaspaceSize=512m ${tasks} -PQA=https://${stand_address} -Pbr=grid -PRETRY=$RETRY -Dorg.gradle.jvmargs=-Xloggc:/abc/logs/gc_regress.log"
            }
        }
    }
}
    ...
    stages {
        stage('Regress') {
            parallel {
                ...
                stage('SCHED8 (Magnit), SCHED13 (magnit), SCHED26 (magnit), SCHED37 (magnit), LIST20 (Magnit), INTEGRATION (Magnit)') {
                    options {
                        timeout(time: 3, unit: 'HOURS')
                    }
                    steps {
                        pl_step('magnit', 'magnitqa-wfm.goodt.tech', 'specific_magnit_LIST specific_magnit_SCHED specific_magnit_SCHED37 specific_magnit_INTEGRATION')
                    }
                    post {
                        always {
                ...
                stage('SCHED2, SCHED9, SCHED10, SCHED11, SCHED12, SCHED21, SCHED22, SCHED24, SCHED32, SCHED36, SCHED41, OTHER13, OTHER20') {
                    options {
                        timeout(time: 5, unit: 'HOURS')
                    }
                    steps {
                        pl_step('pochta1', 'pochta-wfm-release-qa.goodt.tech', 'SCHED2 SCHED9 SCHED10 SCHED11 SCHED12 SCHED21 SCHED22 SCHED24 SCHED32 SCHED36 SCHED41 OTHER13 OTHER20 specific_pochta')
                    }
                    post {
                        always {
                ...
                stage('SCHED7-8, SCHED16-17, SCHED20, SCHED23, SCHED33, SCHED35, SCHED37, SCHED39') {
                    ...
                    steps {
                        pl_step('zozo1', 'zozo-wfm-qa.goodt.tech', 'SCHED7 SCHED8 SCHED16 SCHED17 SCHED20 SCHED23 SCHED33 SCHED35 SCHED37 SCHED39')
                    }
                    post {
                        always {
                            stash_results("zozo1")
                ...
                stage('INTEGRATION, MIX, N2, PS2, AUTH1') {
                    ...
                    steps {
                        testing_step('pochta2', 'pochta-wfm-release-qa.goodt.tech', 'INTEGRATION MIX1 MIX2 MIX3 N2 PS2 AUTH1')
                    }
                    post {
                        always {
                ...
                stage('BC1, FTE, LIST, REP1') {
                    ...
                    steps {
                        testing_step('zozo2', 'zozo-wfm-qa.goodt.tech', 'BC1 FTE2 FTE3 FTE4 FTE7 LIST1 LIST2 LIST6 LIST7 LIST16 REP1')
                    }
                    post {
                        always {
                            stash_results("zozo2")
